...
public class SecurityTest implements TestData {
    ...
    /**
     * This ensures every Rest Method is secured with Method Security. It is very easy to forget
     * securing one method causing a security vulnerability. Feel free to remove / disable / adapt
     * if you do not use Method Security (e.g. if you prefer Web Security to define who may perform
     * which actions) or want to use Method Security on the service layer.
     */
    @Test
    public void ensureSecurityAnnotationPresentForEveryEndpoint() {
        List<Pair<Class<?>, Method>> notSecured =
            components.stream()
                ...
                .filter(
                    clazz ->
                        clazz.getCanonicalName() != null
                            ...
                            .getCanonicalName()
                            .startsWith(
                                BackendApplication.class.getPackageName())) // limit to our package
                .filter(
                    clazz ->
                ...
                .flatMap(
                    clazz ->
                        Arrays.stream(clazz.getDeclaredMethods())
                            .map(
                                method ->
                                    new ImmutablePair<Class<?>, Method>(
                                        clazz, method))) // get all class -> method pairs
                .filter(
                    pair ->
                        Arrays.stream(pair.getRight().getAnnotations())
                            .anyMatch(
                                annotation ->
                                    mappingAnnotations.contains(
                                        annotation
                                            .annotationType()))) // keep only the pairs where the method
                // has a "mapping annotation"
                .filter(
                    pair ->
                        Arrays.stream(pair.getRight().getAnnotations())
                            .noneMatch(
                                annotation ->
                                    securityAnnotations.contains(
                                        annotation
                                            .annotationType()))) // keep only the pairs where the method
        ...
        assertThat(notSecured.size())
            .as(
                "Most rest methods should be secured. If one is really intended for public use, explicitly state that with @PermitAll. "
    ...
    public void givenNoOneLoggedIn_whenFindAll_then401() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(get(MESSAGE_BASE_URI)).andDo(print())
            .andReturn();
        MockHttpServletResponse response = mvcResult.getResponse();
    ...
    public void givenNoOneLoggedIn_whenPost_then403() throws Exception {
        ...
        MvcResult mvcResult =
            this.mockMvc
                .perform(
                    post(MESSAGE_BASE_URI).contentType(MediaType.APPLICATION_JSON).content(body))
                .andDo(print())
                .andReturn();
        MockHttpServletResponse response = mvcResult.getResponse();
